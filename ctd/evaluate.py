...
from eval_utils_fine_tuned import (
    print_args_fine_tuned,
    reset_cfg_fine_tuned,
    extend_cfg_fine_tuned,
    setup_cfg_fine_tuned,
    get_parsed_args_fine_tuned,
)
from eval_utils_adapter import (
    print_args_adapter,
    reset_cfg_adapter,
    extend_cfg_adapter,
    setup_cfg_adapter,
    get_parsed_args_adapter,
)
...
def eval_linear_prob(args, dataset_path, dataset_names, image_extensions, device):
    ...
    weights_dir = f"/home/john/Dev/university/weights/selected_linearprob_models"
    model_names = [
        f"{weights_dir}/CLIP_linear_prob_2_epoch_"
        + args.model
        + "_vit_large_with_augs.pth"
    ]
    model_evaluations = {}
    ...
    for model_name in model_names:
        ...
        for dataset in dataset_names:
            ...
            real_image_directory = (
                dataset_path + "/" + dataset + "/images/val/n01440764/"
            )
            fake_image_directory = (
                dataset_path + "/" + dataset + "/images/val/n01443537/"
            )
            ...
            def is_image(i: str) -> bool:
                return i.endswith((".png", ".jpg", ".jpeg"))

            images = filter(is_image, sorted(images))
...
def eval_fine_tuning(args, dataset_path, dataset_names, image_extensions, device):
    ...
    weights_dir = f"/home/john/Dev/university/weights/selected_finetuned_clip_model"

    if "100k" in args.model:
        model_names = [f"{weights_dir}/finetuned_1_epoch_100k/"]
    elif "80k" in args.model:
        model_names = [f"{weights_dir}/finetuned_1_epoch_80k/"]
    elif "60k" in args.model:
        model_names = [f"{weights_dir}/finetuned_1_epoch_60k/"]
    elif "40k" in args.model:
        model_names = [f"{weights_dir}/finetuned_1_epoch_40k/"]
    elif "20k" in args.model:
        model_names = [f"{weights_dir}/finetuned_1_epoch_20k/"]
...
def eval_adapter_network(args, dataset_path, dataset_names, image_extensions, device):
    ...
    weights_dir = f"/home/john/Dev/university/weights/selected_clip_adapter_models"

    if "100k" in args.model:
        model_names = [f"{weights_dir}/clipadapter_100k_real_fake_04_best/"]
    elif "80k" in args.model:
        model_names = [f"{weights_dir}/clipadapter_40k_real_fake_04/"]
    elif "60k" in args.model:
        model_names = [f"{weights_dir}/clipadapter_30k_real_fake_04/"]
    elif "40k" in args.model:
        model_names = [f"{weights_dir}/clipadapter_20k_real_fake_04/"]
    elif "20k" in args.model:
        model_names = [f"{weights_dir}/clipadapter_10k_real_fake_04/"]
...
def eval_prompt_tuning(args, dataset_path, dataset_names, image_extensions, device):
    ...
    weights_dir = f"/home/john/Dev/university/weights/selected_coop_models"
...
def main(args):
    ...
    if args.variant == "linearProbing":
        ...
        eval_adapter_network(
            args, dataset_path, dataset_names, image_extensions, device
        )
    else:

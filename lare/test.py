...
class ImageDataset(Dataset):
    def __init__(
        ...
        self.albu_pre_train = A.Compose(
            [
                A.PadIfNeeded(
                    min_height=self.data_size, min_width=self.data_size, p=1.0
                ),
...
def train_one_epoch(
    ...
    for images, labels, loss_maps in data_loader:
        ...
        if (
            ...
            lr = get_lr(optimizer)
            logger.info(
                "Ep %03d, it %03d/%03d, lr: %8.7f, CE: %7.6f"
...
def validation_contrastive(model, args, test_file, device, ngpus_per_node):
    ...
    val_dataset = ImageDataset(
        args.data_root, test_file, data_size=args.data_size, split_anchor=False
    )
    if args.distributed:
    ...
    if args.multiprocessing_distributed:
        gt_labels_output_tensor = torch.zeros(
            (len(val_dataset),), dtype=gt_labels.dtype
        ).to(device)
        prob_labels_output_tensor = torch.zeros(
            (len(val_dataset),), dtype=prob_labels.dtype
        ).to(device)
        gt_labels_gathered_tensor_list = list(
            torch.split(gt_labels_output_tensor, len(val_dataset) // 8)
        )
        prob_labels_gathered_tensor_list = list(
            torch.split(prob_labels_output_tensor, len(val_dataset) // 8)
        )
        ...
        prob_labels_list = (
            torch.cat(prob_labels_gathered_tensor_list, dim=0).cpu().numpy()
        )
    else:
...
def main(gpu, ngpus_per_node, args):
    ...
    if args.test_file == "":
        ...
        test_file_list = [
            (
                "/home/users/petterluo/project/FakeImageDetection/outputs/robust_test/anns/midjourney_gaussian3_ann.txt",
                "Midjourney",
            ),
            (
                "/home/users/petterluo/project/FakeImageDetection/outputs/robust_test/anns/sdv4_gaussian3_ann.txt",
                "StableDiffusionV1.4",
            ),
            (
                "/home/users/petterluo/project/FakeImageDetection/outputs/robust_test/anns/sdv5_gaussian3_ann.txt",
                "StableDiffusionV1.5",
            ),
            (
                "/home/users/petterluo/project/FakeImageDetection/outputs/robust_test/anns/adm_gaussian3_ann.txt",
                "ADM",
            ),
            (
                "/home/users/petterluo/project/FakeImageDetection/outputs/robust_test/anns/glide_gaussian3_ann.txt",
                "GLIDE",
            ),
            (
                "/home/users/petterluo/project/FakeImageDetection/outputs/robust_test/anns/wukong_gaussian3_ann.txt",
                "WuKong",
            ),
            (
                "/home/users/petterluo/project/FakeImageDetection/outputs/robust_test/anns/vqdm_gaussian3_ann.txt",
                "VQDM",
            ),
            (
                "/home/users/petterluo/project/FakeImageDetection/outputs/robust_test/anns/biggan_gaussian3_ann.txt",
                "Biggan",
            ),
        ]
    else:
    ...
    if not args.label_smooth:
        ...
        args.criterion_ce = LabelSmoothingLoss(
            classes=args.num_class, smoothing=args.smoothing
        )
    ...
    for epoch in range(args.epoches):
        ...
        for test_file, nickname in test_file_list:
            if args.val_method == "cluster":
                ...
                test_auc, test_acc, test_ap = validation_similarity(
                    model, args, test_file
                )
            else:
                test_auc, test_acc, test_ap, test_raw_acc, test_r_acc, test_f_acc = (
                    validation_contrastive(
                        model, args, test_file, device, ngpus_per_node
                    )
                )
...
if __name__ == "__main__":
    ...
    conf.add_argument(
        "--train_file", type=str, default="annotation/Train_num398700.txt"
    )
    conf.add_argument(
        "--val_file", type=str, default="annotation/Test_MidjourneyV5_num2000.txt"
    )
    conf.add_argument(
        "--test_file", type=str, default="annotation/Test_MidjourneyV5_num2000.txt"
    )
    ...
    conf.add_argument(
        "--num_class", type=int, default=2, help="The class number of training dataset"
    )
    conf.add_argument(
        "--lr", type=float, default=1e-4, help="The initial learning rate."
    )
    conf.add_argument(
        "--weights", type=str, default="out_dir", help="The folder to save models."
    )
    ...
    conf.add_argument(
        "--data_size", type=int, default=448, help="The image size for training."
    )
    ...
    conf.add_argument(
        "--val_method", type=str, default="cluster", choices=["cluster", "sim", "con"]
    )

...
def clip_vit_forward(self, x: torch.Tensor):
    ...
    patch_map = patch_tokens.permute(0, 2, 1).reshape(
        x.shape[0], -1, h, w
    )  # (B, C, H, W)
...
class ROIPooler(nn.Module):
    ...
    def forward(self, feature_map, rois):
        ...
        if self.align:
            return roi_align(
                feature_map, rois, output_size=self.output_size, aligned=self.align
            )
        else:
...
class PoolWithMap(nn.Module):
    ...
    def forward(self, x, loss_map):
        ...
        loss_map = loss_map + self.positional_embedding[:, None, :].to(
            loss_map.dtype
        )  # (HW+1)NC
...
class ScaledDotProductWithMapAttention(nn.Module):
    ...
    def __init__(self, d_in, d_out, d_k, d_v, h, dropout=0.1):
        """
...
class MultiHeadMapAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attention = ScaledDotProductWithMapAttention(
            d_in=self.d_in, d_out=self.d_out, d_k=d_k, d_v=d_v, h=h
        )
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_out)
        self.positional_embedding = nn.Parameter(
            torch.randn(spacial_dim**2 + 1, self.d_in) / self.d_in**0.5
        )  # 197 * d_in

    def forward(
        self, feature_map, loss_map, attention_mask=None, attention_weights=None
    ):
        ...
        loss_map = torch.cat(
            [loss_map.mean(dim=1, keepdim=True), loss_map], dim=1
        )  # bs * 197 * 8
        ...
        if self.identity_map_reordering:
            ...
            out = self.attention(
                q_norm, k_norm, v_norm, loss_map, attention_mask, attention_weights
            )
            out = queries + self.dropout(torch.relu(out))
        else:
            out = self.attention(
                queries, keys, values, loss_map, attention_mask, attention_weights
            )
...
class MultiHeadMapAttentionV2(nn.Module):
    ...
    def __init__(
        ...
        self.identity_map_reordering = identity_map_reordering
        self.attention = ScaledDotProductWithMapAttentionV2(
            d_in=d_in, d_out=d_out, d_k=d_k, d_v=d_v, h=h
        )
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_out)
        ...
        self.pos_embed_kv = nn.Parameter(
            torch.randn(spacial_dim**2 + 1, d_in) / d_in**0.5
        )  # 197 x 1024
        self.pos_embed_q = nn.Parameter(
            torch.randn(spacial_dim**2 + 1, d_in) / d_in**0.5
        )  # 197 x 128
    ...
    def forward(
        ...
        loss_map = self.projection_loss_map_5(
            self.projection_loss_map_4(
                self.projection_loss_map_3(
                    self.projection_loss_map_2(self.projection_loss_map_1(loss_map))
                )
            )
        )
        ...
        loss_map = loss_map + self.pos_embed_q[None, :, :].to(
            loss_map.dtype
        )  # For queries: bs * 197 * 1024
        ...
        if self.ctx_init:
            ...
            if cfg.coop.csc:
                ...
                ctx_vectors = torch.empty(
                    self.num_classes, self.n_ctx, self.ctx_dim, dtype=self.dtype
                )
            else:
